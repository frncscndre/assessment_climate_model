%Load data
load('/home/sushama/wiki/RF_GEMVal_Project/preparedData/ERAPrep_noVeg.mat') %Data from ERA5
load('/home/sushama/wiki/RF_GEMVal_Project/preparedData/GEMPrep_noVeg.mat') %Data from GEM
...
sm_pred_hr = zeros(110,69,183);
sm_pred_smlt = zeros(110,69,183);
sm_pred_sweq = zeros(110,69,183);
sm_pred_tt = zeros(110,69,183);
sm_pred_wa = zeros(110,69,183);

sm_pred_noVeg = zeros(110,69,183);
    ...
    for j = 1:69
        if bbb(i,j) == 1
            ...
            T_gem = table(squeeze(GEM_sm(i,j,:)),squeeze(GEM_hr_1d(i,j,:)),squeeze(GEM_hr_nd(i,j,:)),squeeze(GEM_smlt_1d(i,j,:)),squeeze(GEM_smlt_nd(i,j,:)),...
                ...
                sm_pred_noVeg(i,j,:) = predict(models{i,j},T_gem);
...
clearvars -except sm_pred_noVeg sm_pred_hr sm_pred_smlt sm_pred_sweq sm_pred_tt sm_pred_wa

save /home/sushama/wiki/RF_GEMVal_Project/SSM_predictions/SSMPred_noVeg_RF2.mat