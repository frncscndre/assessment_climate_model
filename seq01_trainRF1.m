...
%nc files contain data for the St. Lawrence Domain (110x69) @30km/home/sushama/wiki/RF_GEMVal_Project/ERA5_Data/
...
ERA_sweq = cat(3,sweq1989,sweq1991,sweq1992,sweq1993,sweq1994,sweq1995,sweq1997,sweq1998,...
    sweq1999,sweq2000,sweq2001,sweq2003,sweq2004,sweq2005,sweq2006,sweq2007,sweq2009,...
...
clay = zeros(110,69,4758);
sand = zeros(110,69,4758);
topo = zeros(110,69,4758);

clay_t = zeros(110,69,1098);
sand_t = zeros(110,69,1098);
topo_t = zeros(110,69,1098);
...
ERA_sweq_t = cat(3,sweq1990,sweq1996,sweq2002,sweq2008,sweq2014,sweq2020);
ERA_hr_t1d = hr(:,:,tag01d);
ERA_tt_t1d = tt(:,:,tag01d);
ERA_wa_t1d = wav(:,:,tag01d);
ERA_smlt_t1d = smlt(:,:,tag01d);

ERA_hr_tnd = movmean(hr(:,:,tag01d),[0,n]);
ERA_tt_tnd = movmean(tt(:,:,tag01d),[0,n]);
ERA_smlt_tnd = movmean(smlt(:,:,tag01d),[0,n]);
ERA_wa_tnd = movmean(wav(:,:,tag01d),[0,n]);

clay_t = reshape(clay_t,[],1);
sand_t = reshape(sand_t,[],1);
topo_t = reshape(topo_t,[],1);

ERA_sm = reshape(ERA_sm,[],1);
ERA_hr_1d = reshape(ERA_hr_1d,[],1);
...
ERA_sm_t = reshape(ERA_sm_t,[],1);
ERA_hr_t1d = reshape(ERA_hr_t1d,[],1);
ERA_hr_tnd = reshape(ERA_hr_tnd,[],1);
ERA_smlt_t1d = reshape(ERA_smlt_t1d,[],1);
ERA_smlt_tnd = reshape(ERA_smlt_tnd,[],1);
ERA_sweq_t = reshape(ERA_sweq_t,[],1);
ERA_tt_t1d = reshape(ERA_tt_t1d,[],1);
ERA_tt_tnd = reshape(ERA_tt_tnd,[],1);
ERA_wa_t1d = reshape(ERA_wa_t1d,[],1);
ERA_wa_tnd = reshape(ERA_wa_tnd,[],1);
...
clearvars -except ERA_sm ERA_hr_1d ERA_hr_nd ERA_smlt_1d ERA_smlt_nd ERA_sweq ERA_tt_1d ERA_tt_nd ERA_wa_1d ERA_wa_nd clay sand topo ...
                  ERA_sm_t ERA_hr_t1d ERA_hr_tnd ERA_smlt_t1d ERA_smlt_tnd ERA_sweq_t ERA_tt_t1d ERA_tt_tnd ERA_wa_t1d ERA_wa_tnd clay_t sand_t topo_t

disp('Merging tables')
%Arranges data in tables to train RF model
T_training = table(ERA_sm,ERA_hr_1d,ERA_hr_nd,ERA_smlt_1d,ERA_smlt_nd,ERA_sweq,ERA_tt_1d,ERA_tt_nd,ERA_wa_1d,ERA_wa_nd,clay,sand,topo,...
    'VariableNames',{'sm','hr_l01','hr_lag','smlt_l01','smlt_lag','sweq','tt_l01','tt_lag','wa_l01','wa_lag','clay','sand','topo'});
T_testing = table(ERA_sm_t,ERA_hr_t1d,ERA_hr_tnd,ERA_smlt_t1d,ERA_smlt_tnd,ERA_sweq_t,ERA_tt_t1d,ERA_tt_tnd,ERA_wa_t1d,ERA_wa_tnd,clay_t,sand_t,topo_t,...
    'VariableNames',{'sm','hr_l01','hr_lag','smlt_l01','smlt_lag','sweq','tt_l01','tt_lag','wa_l01','wa_lag','clay','sand','topo'});

clearvars -except T_testing T_training nn n

%Define RF parameters
minLeafSize = 1000; %Define minimum leaf size here
numTrees = 100; %Define number of trees here
ibf = 0.9; %Define in-bag fraction here

%Naming convention for models and SSM predictions
vecstr = num2str(minLeafSize);
name = strcat('RF1_ModWholeCell_MLS',vecstr);
mls = strcat('MinLeafSize = ',32,vecstr);
workspc = strcat('RFM_ModWhoCell_MLS',vecstr,'.mat');
disp(strcat('mls = ',num2str(minLeafSize)));

disp('Training RF1 model')
...
predImp = forest.OOBPermutedPredictorDeltaError; %Saves predictor importance values

sm_predicted = predict(forest,T_testing);
sm_pred_test = sm_predicted;
            
sm_predicted2 = predict(forest,T_training);
sm_pred_train = sm_predicted2;

disp(strcat('Saving ',name,'_SavedModel','.mat'))
save(workspc,'predImp','sm_pred_test','sm_pred_train') %Saves SSM predictions and importances
save(strcat('/home/sushama/wiki/RF_GEMVal_Project/models/',name,'_SavedModel','.mat'),'forest','-v7.3') %Saves RF1 model